...
class TestIO(unittest.TestCase):
    ...
    def setUp(self):
        ...
        df = read_path_csv(self.csv_filename)
        self.df = df
        self.n = len(df)
    ...
    def test_paths_from_df(self):
        """Load paths from DataFrame."""
        path = paths_from_df(self.df, lexico=False)
        self.assertEqual(
            next(path).tolist(), [[44, 10], [44, 10], [44, 11], [44, 11], [45, 11]]
        )
        self.assertEqual(
    ...
    def test_paths_from_df_lexico(self):
        ...
        path = paths_from_df(self.df, lexico=True, grid_width=self.grid_width)
        self.assertEqual(next(path).tolist(), [744, 744, 814, 814, 815])
        self.assertEqual(next(path).tolist(), [744, 814, 814, 814, 815])
        # I've added a duplicated line (2nd attempt)
        self.assertEqual(next(path).tolist(), [744, 814, 814, 814, 815])
